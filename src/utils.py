...
def most_recent_indices_to_ptr(size_indices, ptr, buf_size_now, total_size, idxs_valid=None):
        if idxs_valid is None:
            idxs_valid = np.arange(buf_size_now)
        assert np.all(np.diff(idxs_valid) >= 0), "idxs_valid must be sorted in ascending order"
        idxs_valid_indices = np.arange(len(idxs_valid))
        """
        idxs_sorted = np.argsort(ptr-idxs_valid)
        print("UY", idxs_sorted, idxs_valid, len(idxs_valid),  ptr)
        
        
        closest_idxs = idxs_sorted[:size_indices] if len(idxs_sorted) >= size_indices else idxs_sorted
        print("OUTPUT", closest_idxs, size_indices, total_size)
        """
        if size_indices < buf_size_now-1:
            closest_idxs = np.arange(len(idxs_valid))
            assert max(idxs_valid) < buf_size_now
            
        idxs_valid_indices = np.sort(idxs_valid_indices)
        before_ptr = idxs_valid_indices[idxs_valid <= ptr]
        if len(before_ptr) >= size_indices:
            closest_idxs = before_ptr[-size_indices:]
            assert len(closest_idxs) == min(size_indices, len(idxs_valid))
        else:
            before_end = idxs_valid_indices[idxs_valid > ptr][-(size_indices-len(before_ptr)):]
            closest_idxs = np.concatenate([before_ptr, before_end])
            assert len(closest_idxs) == min(size_indices, len(idxs_valid))
        assert max(closest_idxs) < len(idxs_valid)

        
        return closest_idxs, idxs_valid[closest_idxs]
