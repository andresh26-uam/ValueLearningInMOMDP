...
class ProxyPolicy(Policy):
    ...
    def parameters(self):
        ...
        if isinstance(self.agent_mobaselines, Envelope):
            ...
            base['params'] = dict({k: list(v.policy.parameters(
            )) for k, v in self.agent_mobaselines.learner_per_align_func.items()})
        else:
            raise NotImplementedError(
                f"Unsupported agent_morl type: {type(self.agent_mobaselines)}")
        return base
    ...
    def __init__(self, agent_morl: MOAgent, weight_default=None, epsilon=0.1):
        ...
        if weight_default is not None:
            ...
            self.set_weight(np.ones(agent_morl.reward_dim) /
                            agent_morl.reward_dim)
...
class WeightsToAlgos(object):
    ...
    def compare_parameters(self, other):
        ...
        for k, v in self.weights_to_index.items():
            if k not in other.weights_to_index or other.weights_to_index[k] != v:
                print("Keys or values do not match:", k, v,
                      other.weights_to_index.get(k, None))
                return False
        for algo1, algo2 in zip(self.algorithm_dict.values(), other.algorithm_dict.values()):
            if algo1.__class__ != algo2.__class__:
                print("Algorithms do not match:",
                      algo1.__class__, algo2.__class__)
                return False
    ...
    def __init__(self, weights_to_algos: Dict[str, Agent]):
        ...
        for k, v in weights_to_algos.items():
            if not isinstance(v, Agent):
                raise ValueError(
                    f"Expected MOAgent for key {k}, got {type(v)}")
            self.weights_to_index[transform_weights_to_tuple(
                k, self.size_weights)] = counter
    ...
    def get_algo(self, weight_or_index: Union[int, tuple]) -> Agent:
        ...
        weights_real = transform_weights_to_tuple(
            weight_or_index, self.size_weights)
    ...
    def contains(self, weight_or_index: Iterable) -> bool:
        ...
        if isinstance(weight_or_index, int):
            ...
            if a:
                assert self.weights_to_index.values().count(
                    weight_or_index) == 1, "Inconsistent state in WeightsToAlgos"
        else:
            weights_real = transform_weights_to_tuple(
                weight_or_index, self.size_weights)
            a = weights_real in self.weights_to_index.keys()
            if a:
                assert self.algorithm_dict.get(
                    self.weights_to_index[weights_real], None) is not None, "Inconsistent state in WeightsToAlgos"
        return a
    ...
    def add_algo(self, weight: Iterable, algo: Agent, warn_override=False, override_existent=None):
        ...
        if self.contains(weights_real):
            if warn_override:
                raise Warning(
                    f"Warning: Overriding existing algorithm for weight {weight}")
            self.algorithm_dict[self.weights_to_index[weights_real]] = algo

        else:
            if override_existent is not None:
                assert isinstance(
                    override_existent, int), "override_existent must be an integer index"
                if override_existent in self.weights_to_index.values():
                    self.weights_to_index = {k: v for k, v in self.weights_to_index.items() if int(
                        v) != int(override_existent)}
...
class MOBaselinesAgent(Agent, Serializable):
    ...
    def __init__(self, env: gym.Env, eval_env: gym.Env, agent_class: type[MOAgent], agent_kwargs: Dict, mdp_info:  MDPInfo, is_single_objective: bool, weights_to_sample: Optional[List[Tuple]] = None,
        ...
        assert not isinstance(
            env, MushroomEnvironment), "Invalid environment type."
        ...
        if weights_to_sample is None or len(weights_to_sample) == 0:
            weights_to_sample = [np.ones((env.get_wrapper_attr(
                'reward_dim'),)) / float(env.get_wrapper_attr('reward_dim'))]
        ...
        if self.is_single_objective:
            ...
            for w in weights_to_sample:
                ...
                self.weights_to_algos.add_algo(
                    w, agent_mobaselines, warn_override=True)
        ...
        self.agent_mobaselines = agent_mobaselines
        self.current_weights = weights_to_sample[0] if weights_to_sample is not None else np.ones(
            (env.get_wrapper_attr('reward_dim'),)) / float(env.get_wrapper_attr('reward_dim'))
        self.policy = ProxyPolicy(
            self.agent_mobaselines, weight_default=weights_to_sample[0], epsilon=0.0)
    ...
    def save_agent(self, agent_mobaselines: MOAgent, base_dir='', identification='main'):
        ...
        if isinstance(agent_mobaselines, ValueSystemLearningPolicy):
            ...
            path = path + '_mo'
            assert isinstance(agent_mobaselines,
                              MOAgent), "Agent must be a MOAgent."
            agent_mobaselines.save(
                save_dir=path, filename=f'file', save_replay_buffer=False)
    ...
    def load_agent(self, base_dir: str, identification='main', ref_env: gym.Env = None, weights=None):
        ...
        if os.path.exists(path_t + '_st'):
            assert issubclass(self.agent_class, ValueSystemLearningPolicy), "real type: {}".format(
                self.agent_class.__name__)  # type: ignore

            agent_morl = self.agent_class.load(
                ref_env=ref_env, path=path_t + '_st')

        else:
            assert os.path.exists(
                path_t + '_mo'), f"Path {path_t} does not exist."
            path_t = path_t + '_mo'
            args = deepcopy(self.agent_kwargs)
            args.update({'weights': weights})
            const_kwargs = get_signature_params(
                self.agent_class, self.agent_kwargs)
            ...
            if os.path.exists(path_t):
                ...
                if 'envelope' in self.name.lower():
                    ...
                    raise FileNotFoundError(
                        f"Could not load, unrecognized agent: {self.name}, at {path_t}")
        return agent_morl
    ...
    def save_zip(self, zip_file: ZipFile, full_save, folder=''):
        ...
        ff = os.path.join(os.path.dirname(
            zip_file.filename), f'{self.name}_agent')
        os.makedirs(ff, exist_ok=True)

        self.save_agent(self.agent_mobaselines,
                        base_dir=ff, identification='main')
        if self.is_single_objective:
            # save the weights to algos
            for windex, algo in self.weights_to_algos.algorithm_dict.items():
                print("SAVED AGENT, ", windex)
                self.save_agent(algo, base_dir=ff,
                                identification=f'a_{windex}')
    ...
    def load(env, eval_env, path, name=None):
        ...
        agent_morl_main = self.load_agent(
            base_dir=ff, identification='main', ref_env=env)
        ...
        if self.is_single_objective:
            ...
            for weight, index_ in self.weights_to_algos.weights_to_index.items():
                agent_morl = self.load_agent(
                    base_dir=ff, identification=f'a_{index_}', ref_env=env, weights=weight)
    ...
    def set_weights(self, weights, override_existent: int = None):
        ...
        if self.is_single_objective:
            ...
            if not self.weights_to_algos.contains(weights):
                self.weights_to_algos.add_algo(weights, self.agent_class(
                    env=self.env, **uparams), warn_override=True, override_existent=override_existent)
    ...
    def set_reward_vector_function(self, reward_vector, warn_relabel_not_possible=True):
        ...
        if self.is_single_objective:
            for windex, algo in self.weights_to_algos.algorithm_dict.items():
                algo.set_reward_vector_function(
                    reward_vector, warn_relabel_not_possible=warn_relabel_not_possible)
        else:
            if hasattr(self.agent_mobaselines, 'set_reward_vector_function'):
                self.agent_mobaselines.set_reward_vector_function(
                    reward_vector, warn_relabel_not_possible=warn_relabel_not_possible)
    ...
    def fit(self, dataset=None, **info):
        ...
        if self.is_single_objective:
            ...
            self.agent_mobaselines = self.weights_to_algos.get_algo(
                self.get_weights())
            self.agent_mobaselines.set_weight(self.get_weights())

            print("Training agent with weights:", weight)
            print(
                [p.__class__.__name__ for p in self.weights_to_algos.algorithm_dict.values()])
        else:
            weight = None

        train_kwargs = get_signature_params(
            self.agent_mobaselines.train, self.train_kwargs)

        self.policy.set_agent_baselines(self.agent_mobaselines)
        self.policy.set_weight(self.get_weights())
        print("Training agent with weights:", self.get_weights())
        if isinstance(self.policy.weights, th.Tensor):
            assert np.allclose(self.policy.weights.detach().cpu(
            ).numpy(), self.get_weights().detach().cpu().numpy())
        else:
            assert np.allclose(self.policy.weights, self.get_weights())
        # self.policy.set_epsilon(0.5)
        self.agent_mobaselines.train(
            eval_env=self.eval_env, weight=weight, **train_kwargs)
...
"""
    ...
    def pareto_front(self, num_eval_episodes_for_front=1000, num_eval_weights_for_front=100, discount=1.0, use_weights=None):
        ...
        if use_weights is not None:
            ...
            if self.is_single_objective:
                eval_weights = list(
                    self.weights_to_algos.weights_to_index.keys())
            else:

                eval_weights = equally_spaced_weights(
                    self.agent_mobaselines.reward_dim, n=num_eval_weights_for_front)
...
def obtain_trajectory_and_eval_mo(
    ...
    vec_return, disc_vec_return = np.zeros(
        (size_weights, ), dtype=np.float32), np.zeros((size_weights, ), dtype=np.float32)
    ...
    w_eval = np.asarray(
        w_eval, dtype=np.float32) if w_eval is not None else None
    ...
    while not (terminated or truncated):
        ...
        assert len(
            r.shape) == 1, f"Reward shape {r.shape} should be 1-dimensional"
    ...
    return TrajectoryWithValueSystemRews(obs=np.array(observations),
                                         ...
                                         v_rews_real=np.array(v_rews_real).T if len(
                                             v_rews_real) > 0 else None,
                                         rews_real=np.array(rews_real) if len(
                                             rews_real) > 0 else None,
...
def obtain_trajectories_and_eval_mo(
    ...
    if reward_vector is not None:
        ...
        agent.set_reward_vector_function(
            reward_vector, warn_relabel_not_possible=warn_relabel_not_possible)
        collect_real_reward_only = False
    else:
