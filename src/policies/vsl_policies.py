...
from src.utils import NpEncoder, deconvert, deserialize_policy_kwargs, import_from_string, serialize_policy_kwargs
...
class MaskedPolicySimple(MlpPolicy):
    ...
    def extract_mask(self, obs):
        ...
        np.testing.assert_array_less(
            self.obs_mask, 1.0001, err_msg="Action masks should be in [0, 1] range")
        np.testing.assert_array_less(-0.0001, self.obs_mask,
                                     err_msg="Action masks should be in [0, 1] range")
    ...
    def _get_action_dist_from_latent(self, latent_pi: torch.Tensor) -> Distribution:
        ...
        HUGE_NEG = torch.tensor(
            -1e8, dtype=distribution.distribution.logits.dtype, device=latent_pi.device)

        distribution.distribution.logits = torch.where(
            self.obs_mask == 1.0,  distribution.distribution.logits, HUGE_NEG)
        return distribution
...
class ValueSystemLearningPolicy():
    ...
    def set_env(self, env: ValueAlignedEnvironment):
        if env.has_wrapper_attr('set_weight') or hasattr(env, 'set_weight'):
            ...
            self.env = LinearTorchReward(env, weight=torch.ones((env.get_wrapper_attr(
                'reward_space').shape[0], ), dtype=torch.float32)/env.get_wrapper_attr('reward_space').shape[0], dtype=torch.float32)
    ...
    def set_weight(self, weight: np.ndarray):
        ...
        if not (self.env.has_wrapper_attr('set_weight') or hasattr(self.env, 'set_weight')):
            self.env = LinearTorchReward(
                self.env, weight=util.safe_to_tensor(weight), dtype=torch.float32)
    ...
    def set_reward_vector_function(self, reward_vector_function: Callable[[torch.Tensor], torch.Tensor]):
        ...
        if self.env.has_wrapper_attr('set_reward_vector_function'):
            self.env.get_wrapper_attr('set_reward_vector_function')(
                reward_vector_function)
        else:
            self.env = RewardVectorFunctionWrapper(
                self.env, reward_vector_function)
        ...
        if only_states:
            ...
            while not ((terminated or truncated) and end_trajectories_when_ended) and (t_max is None or (t < t_max)):
                ...
                if with_reward:
                    ...
                    ), obs_in_state, action, next_state=next_obs_in_state, done=terminated, info=info_next, custom_grounding=self.env.get_grounding_func())
    ...
    def calculate_value_grounding_expectancy(self, value_grounding: Callable[[Any, Any, Any, bool, int | str], Any],
        ...
        if initial_state_distribution is not None:
            ...
            precalc = np.zeros(
                (*(value_grounding(vi=0).shape), self.env.n_values), dtype=np.float64)
        ...
        for t in trajs:
            ...
            for i, (to, ta, tn) in enumerate(zip(t.obs[:-1], t.acts, t.obs[1:])):
                ...
                if precalc is not None:
                    ...
                    expected_gr += (np.reshape(value_grounding(to, ta, tn, None,
                                    # TODO terminal?
                                                               vi='vg'), (self.env.n_values, ))) / len(t)
    ...
    def learn(self, alignment_function, grounding_function, reward=None, discount=1.0, stochastic=True, eval_env=None, **kwargs):
        raise NotImplementedError(
            "Learning not implemented for this VSL policy subclass: " + self.__class__.__name__)

    def train(self, eval_env, weight=None, reward_vector=None, **kwargs):
        # self.set_env(env)
        self.learn(alignment_function=weight, eval_env=eval_env,
                   grounding_function=reward_vector, **kwargs)
...
class LearnerValueSystemLearningPolicy(ValueSystemLearningPolicy, BasePolicy):
    ...
    def learn(self, alignment_function, eval_env=None, grounding_function=None, reward=None, discount=1.0, stochastic=True,
        ...
        tuple_alignment_function = transform_weights_to_tuple(
            alignment_function)
        learner = self.get_learner_for_alignment_function(
            tuple_alignment_function, set_env=True)
        ...
        if self.env.has_wrapper_attr('get_grounding_func'):
            prev_grounding_function = self.env.get_wrapper_attr(
                'get_grounding_func')()
        ...
        if self.env.has_wrapper_attr('get_grounding_func'):
            self.env.get_wrapper_attr('set_grounding_func')(
                prev_grounding_function)
    ...
    def set_reward_vector_function(self, reward_vector_function: Callable[[torch.Tensor], torch.Tensor], warn_relabel_not_possible=True):
        ...
        for alignment, learner in self.learner_per_align_func.items():
            ...
            if warn_relabel_not_possible:
                assert isinstance(
                    learner.rollout_buffer, RolloutBufferCustomReward), "Learner must be a BaseAlgorithm instance"
            if not hasattr(learner.rollout_buffer, 'set_reward_vector_function'):
                new_buffer = RolloutBufferCustomReward(
                    learner.rollout_buffer.buffer_size, learner.observation_space, learner.action_space, relabel_experience=True)
            ...
            learner.rollout_buffer.set_reward_vector_function(
                reward_vector_function)
    ...
    def load(ref_env, path='learner_dummy'):
        ...
        init_params['stochastic_eval'] = bool(
            init_params.get('stochastic_eval', False))
        ...
        if 'rollout_buffer_class' in init_params['learner_kwargs']:
            init_params['learner_kwargs']['rollout_buffer_class'] = ROLLOUT_BUFFER_CLASSES[init_params['learner_kwargs']
            ...
            assert issubclass(init_params['learner_kwargs']['rollout_buffer_class'],
                              RolloutBufferCustomReward), "Rollout buffer class must be a subclass of RolloutBufferCustomReward"
        ...
        for file_name in os.listdir(save_folder):
            if file_name.startswith("alignment_"):
                ...
                assert isinstance(
                    alignment, tuple),  "Alignment function must be a tuple"
    ...
    def __init__(self, *args, env: ValueAlignedEnvironment, stochastic_eval=False, learner_class: BaseAlgorithm, learner_kwargs, policy_class=MlpPolicy, policy_kwargs={}, observation_space=None, action_space=None,  **kwargs):
        super().__init__(*args, env=env, stochastic_eval=stochastic_eval,
    ...
    def obtain_trajectory(self, alignment_func_in_policy=None, seed=32, **kwargs):
        ...
        traj = super().obtain_trajectory(
            alignment_func_in_policy=alignment_func_in_policy, seed=seed, **kwargs)
    ...
    def create_algo(self, environ=None, alignment_function=None):
        ...
        ret = self.learner_class(policy=self.policy_class, env=environ,
                                 policy_kwargs=pkwargs_new, **self.learner_kwargs)
    ...
    def act(self, state_obs, policy_state=None, exploration=0, stochastic=True, alignment_function=None):
        self._alignment_func_in_policy = transform_weights_to_tuple(
            alignment_function)
    ...
    def act_and_obtain_action_distribution(self, state_obs, policy_state=None, exploration=0, stochastic=True, alignment_function=None):
        ...
        assert self.observation_space.contains(
            state_obs), f"State observation not in observation space: {state_obs} not in {self.observation_space}"
        ...
        if np.random.rand() > exploration:
            ...
            base_distribution = np.ones(
                self.action_space.n) / self.action_space.n
...
class VAlignedDiscretePolicy(ValueSystemLearningPolicy):
    ...
    def act(self, state_obs: Any, policy_state=None, exploration=0, stochastic=True, alignment_function=None):
        ...
        probs = self.get_policy_probs(
            policy=policy, state_obs=state_obs, policy_state=policy_state)
        do_explore = False
        if np.random.rand() < exploration:
            do_explore = True
...
class VAlignedDiscreteDictPolicy(VAlignedDiscretePolicy):
    ...
    def __init__(self, policy_per_va_dict, env,  expose_state=True, assume_env_produce_state=True, *args, **kwargs):
        ...
        def policy_per_va(x): return self.policy_per_va_dict[x]

        super().__init__(policy_per_va, env,  expose_state,
                         assume_env_produce_state, *args, **kwargs)
    ...
    def calculate_value_grounding_expectancy_precise(self, value_grounding: Callable[[np.ndarray, np.ndarray], float], policy_align_func, n_seeds=100, n_rep_per_seed=10, exploration=0, stochastic=True, t_max=None, seed=None, p_state=None, env_seed=None, options=None, initial_state_distribution=None):
        ...
        precalc = np.zeros((*(value_grounding(vi=0).shape),
                           self.env.n_values), dtype=np.float64)
        # print("POLICY IN PREC", pi[150:200])
        reward_matric_precalc = np.zeros(
            (*(value_grounding(vi=0).shape), self.env.n_values), dtype=np.float64)
        for vi in range(self.env.n_values):
            precalc[:, :, vi] = value_grounding(vi=vi)
            reward_matric_precalc[:, :, vi] = self.env.reward_matrix_per_align_func(
                self.env.basic_profiles[vi])
        for t in range(self.env.horizon):

            pol_t = pi if len(pi.shape) == 2 else pi[t]
            if not stochastic:
                pol_t = concentrate_on_max_policy(
                    pol_t, distribute_probability_on_max_prob_actions=False, valid_action_getter=lambda s: self.env.valid_actions(s, None))
...
class VAlignedDictSpaceActionPolicy(VAlignedDiscreteDictPolicy):
    ...
    def learn(self, alignment_function=None, grounding_function=None, reward=None, discount=1.0, stochastic=True, policy_approximation_method=None, **kwargs):
        ...
        # print("REWARD", reward)
        if reward is None:
            reward_matrix = self.env.reward_matrix_per_align_func(
        ...
        if isinstance(policy_approximation_method, str):
            policy_approximation_method = PolicyApproximators(
                policy_approximation_method)
        _, _, pi = mce_partition_fh(self.env, reward=reward_matrix, discount=discount,
                                    approximator_kwargs=kwargs['approximator_kwargs'], deterministic=not stochastic, policy_approximator=policy_approximation_method)
...
class ValueSystemLearningPolicyCustomLearner(VAlignedDiscreteDictPolicy):
    ...
    def __init__(self, *args, policy_per_va_dict, env, learner_method: Callable[[Any], Any], saved_methods_per_va_dict=None, expose_state=True, assume_env_produce_state=False, stochastic_eval=False, **kwargs):
        ...
        self.saved_policies_per_va_dict = dict(
        ) if saved_methods_per_va_dict is None else saved_methods_per_va_dict

    def learn(self, alignment_function=None, grounding_function=None, reward=None, discount=1, stochastic=True, **kwargs):

        v, q, policy, save_file, save_m, load_m = self.learner_method(
            environment=self.env, alignment_function=alignment_function, grounding_function=grounding_function, reward=reward, discount=discount, stochastic=stochastic, **kwargs)
