...
def probability_BT(x: th.Tensor, y: th.Tensor, threshold=50.0, with_logits=False) -> th.Tensor:
    ...
    with th.no_grad():
        ...
        assert not any(x.isnan()) and not any(
            x.isinf()), f"Input tensor x contains NaN or Inf values: {x}"
        assert not any(y.isnan()) and not any(
            y.isinf()), f"Input tensor y contains NaN or Inf values: {y}"
        assert not any(returns_diff.isnan()) and not any(returns_diff.isinf(
        )), f"returns_diff is NaN or Inf. rews1: {x}, rews2: {y}, returns_diff: {returns_diff}"
        assert not any(ret.isnan()) and not any(
            ret.isinf()), f"Output prob ret contains NaN or Inf values: {ret}"
    ...
    # assert th.isnan(y).sum() == 0, f"Input tensor y contains NaN values: {y}"
        ...
        assert th.allclose(ret, 1.0/(1.0 + th.exp(-returns_diff))
                           ), f"oh, {ret} vs {1.0/(1.0 + th.exp(-returns_diff))}"
        assert isinstance(
            ret, th.Tensor), f"Expected th.Tensor, got {type(ret)}"
...
def calculate_accuracies(probs_vs: th.Tensor = None, probs_gr: th.Tensor = None, gt_probs_vs: th.Tensor = None, gt_probs_gr: th.Tensor = None, indifference_tolerance=0.0, reduce='mean', return_TAC=False):
    ...
    if probs_vs is not None:
        ...
        misclassified_vs = ~(wellclassified_positive |
                             wellclassified_negative | wellclassified_indifferent)
...
def likelihood_x_is_target(pred_probs, target_probs, mode='numpy', slope=1, adaptive_slope=True, qualitative_mode=False, indifference_tolerance=0.0):
    ...
    if qualitative_mode:
        probs = th.zeros_like(
            pred_probs) if mode == 'th' else np.zeros_like(pred_probs)
...
"""
    ...
    def probability(self, rews1: th.Tensor, rews2: th.Tensor) -> th.Tensor:
        ...
        assert not any(returns_diff.isnan(
        )), f"returns_diff is NaN. rews1: {rews1}, rews2: {rews2}, returns_diff: {returns_diff}"
...
class BaseVSLClusterRewardLoss(preference_comparisons.RewardLoss):
    ...
    def loss_func(self, probs, target_probs, misclassified_pairs=None, apply_on_misclassified_only=False, logits=None):
        if misclassified_pairs is not None and apply_on_misclassified_only:
            ...
            logits_l = logits[misclassified_pairs ==
                              True] if logits is not None else None
        else:
            probs_l = probs
            logits_l = logits
        ...
        if len(probs_l) == 0:
            raise ValueError(
                "No probabilities to compute loss on. Check if the misclassified pairs are correct or if the probabilities are empty.")
        ...
        if self.label_smoothing > 0.0:
            with th.no_grad():
                ...
                target_probs_l = target_probs_l * \
                    (1.0 - self.label_smoothing) + 0.5 * self.label_smoothing
        weights = None
        ...
        if self.missclassification_min_weighting < 1.0 and not apply_on_misclassified_only:
            ...
            closs = th.nn.functional.binary_cross_entropy_with_logits(
                # /sum(weights)
                logits_l, target_probs_l, reduction='none', weight=weights)
        else:
            closs = th.nn.functional.binary_cross_entropy_with_logits(
                logits_l, target_probs_l, reduction='none', weight=weights)
        ...
        if self.confident_penalty > 0.0:
            p = self.confident_penalty * \
                th.multiply(probs_l, th.log(
                    th.max(probs_l, th.full_like(probs_l, 1e-5))))
            assert not any(
                p.isnan()), f"probs_l: {probs_l}, target_probs_l: {target_probs_l}"
    ...
    def simple_forward(self, reward_vector_module: RewardVectorModule, weights_per_cluster: Dict[int, LinearAlignmentLayer], fragments1, fragments2, cluster_fidxs):
        ...
        for vi in range(reward_vector_module.num_outputs):
            pbt, logits = probability_BT(
                grounding1[:, vi], grounding2[:, vi], with_logits=True)
        ...
        for cluster, fidxs in enumerate(cluster_fidxs):
            ...
            probs, logits = probability_BT(
                w.forward(grounding1[fidxs]), w.forward(grounding2[fidxs]), with_logits=True)
            assert probs.shape[0] == probs_value_system[fidxs].shape[0], (
                probs.shape, probs_value_system[fidxs].shape)
    ...
    def forward(
        ...
        if self.per_agent:
            for ag, fidxs_ag in fidxs_per_agent.items():
                if len(fidxs_ag) == 0:
                    raise ValueError(
                        f"Agent {ag} has no fragments to compute loss on. Check if the agent has fragments in the dataset or if the fidxs_per_agent is correct.")

                loss_vs += self.loss_func(probs_vs[fidxs_ag], preferences[fidxs_ag], misclassified_pairs=missclassified_vs[fidxs_ag],
                                          apply_on_misclassified_only=self.vs_apply_on_misclassified_only, logits=logits_value_system[fidxs_ag])
            loss_vs /= n_agents
        else:
            loss_vs = self.loss_func(probs_vs, preferences, missclassified_vs,
                                     apply_on_misclassified_only=self.vs_apply_on_misclassified_only, logits=logits_value_system)
        if loss_vs > 1e+35:
            print(
                f"Large loss vs: {loss_vs}, probs_vs: {probs_vs}, preferences: {preferences}, missclassified_vs: {missclassified_vs}, logits_value_system: {logits_value_system}")
        ...
        if self.cluster_similarity_penalty > 0.0:
            ...
            print(
                f"Conciseness penalty: {self.cluster_similarity_penalty, conc_penalty}, reduction: {self.conciseness_penalty_reduction}")
        ...
        if self.per_agent:
            for ag, fidxs_ag in fidxs_per_agent.items():
                ...
                for vi in range(probs_gr.shape[1]):
                    ...
                    loss_vi_ag = self.loss_func(probs_ag_v[:, vi], preferences_with_grounding_ag[:, vi], misclassified_pairs=missclassified_gr_ag[vi],
                                                apply_on_misclassified_only=self.gr_apply_on_misclassified_only, logits=logits_ag[:, vi])
    ...
    def conciseness_penalty(self, grounding1: th.Tensor, grounding2: th.Tensor,
        ...
        for (c1, vs1), (c2, vs2) in itertools.combinations(list(enumerate(value_system_network_per_cluster)), 2):
            ...
            if OPTION3:
                jsd12 = jensen_shannon_pairwise_preferences(
                    preferences[cfidxs1], model2_in1[cfidxs1])
                jsd21 = jensen_shannon_pairwise_preferences(
                    preferences[cfidxs2], model1_in2[cfidxs2])

                result.append((jsd12 + jsd21)*1.0/(len(ac1) + len(ac2)))
            else:
                result.append(jensen_shannon_pairwise_preferences(
                    model1_in2, model2_in1))
        ...
        if conc_penalty_reduction == 'min':
            ...
            def conc_penalty_reduction(x, dim): return x
        elif conc_penalty_reduction == 'sum':
            conc_penalty_reduction = th.sum
        else:
            raise ValueError(
                f"Unknown conc_penalty_reduction: {conc_penalty_reduction}. Must be 'min' or 'mean'.")
...
"""
    ...
    def forward(self, fragments1, fragments2, reward_vector_module, weights_per_cluster, cluster_fidxs, preferences, preferences_with_grounding, fidxs_per_agent):
        lossMetrics = super().forward(fragments1, fragments2, reward_vector_module, weights_per_cluster,
                                      cluster_fidxs, preferences, preferences_with_grounding, fidxs_per_agent)
                ...
                for vi in range(len(self.lambdas)):
                    if self.lambdas[vi] > self.initial_lambda:
                        ...
                        self.lambdas[vi].data = th.clamp(
                            self.lambdas[vi].data - decay, min=self.initial_lambda)
    ...
    def gradients(self, scalar_loss: th.Tensor, renormalization: float) -> None:
        ...
        with th.no_grad():
            ...
            for vi, l in enumerate(self.lagrange_multipliers):
                l.grad = None
                diff = abs(
                    self.last_accuracy_gr[vi] - self.best_accuracies[vi])
                if self.last_accuracy_gr[vi] >= self.best_accuracies[vi]:
                    self.best_accuracies[vi] = 0.05*float(
                        self.last_accuracy_gr[vi]) + 0.95*self.best_accuracies[vi]
                if diff > max_diff:
                    max_diff = diff
                    farthest_l = vi
