...
class PVSL(object):
        ...
        with tqdm(total=initial_reward_learning_iterations, desc="Initial Reward Learning Iterations",
            ...
            for it in range(initial_reward_learning_iterations):
                ...
                for cycle in range(em_cycles_per_iteration):
                    ...
                    if ((not mutated) or cycle >= 1) or (condition_using_base and disable_selection_after_initial_reward_iterations) or condition_avoid_selection:
    ...
    def _pbmorl_reward_train_callback(self, base_dataset,
        ...
        val_set = base_dataset
        assert self.static_weights

        self.current_assignment.grounding = self.train_reward_net
        self.update_training_networks_from_assignment(
            self.value_system_per_cluster, self.train_reward_net, self.current_assignment, only_grounding=True)
        self.current_assignment.weights_per_cluster = self.value_system_per_cluster
        assert len(
        ...
        check_grounding_value_system_networks_consistency_with_optim(
            self.train_reward_net, self.value_system_per_cluster, self.optim, only_grounding=True)
                    ...
                    for cl2, ags2 in enumerate(assignment_vs):
                        ...
                        if np.max(abs(vsc - vsc2)) <= 0.05:
                            ...
                            if len(ags) >= len(ags2):  # we choose the most popular weights
                                ...
                                # print(assignment_vs)
                                # print("BEFORE", self.value_system_per_cluster[cl2])
                                ...
                                # print("AFTER", self.value_system_per_cluster[cl2])
                            else:
            ...
            for i in range(n_optim_steps):
                ...
                with th.no_grad():
                    ...
                    dataset_b, fids_per_agent = dataset.select_batch(
                        batch_size_reward_buffer, transitions=False, device=self.train_reward_net.device, per_agent=False)
                ...
                anames = [ag for ag in unique_agent_ids]
