...
from src.algorithms.clustering_utils_simple import COLOR_PALETTE, generate_mutated_assignment
...
from src.policies.morl_custom_reward import EnvelopePBMORL, MORecordEpisodeStatisticsCR
from src.policies.mushroom_agent_mobaselines import MOBaselinesAgent
from src.reward_nets.vsl_reward_functions import ConvexAlignmentLayer, EnsembleRewardVectorModule, RewardVectorModule, VectorModule, LinearAlignmentLayer, create_alignment_layer
import torch as th
...
class PVSL(object):
    ...
    def __init__(self, Lmax, mobaselines_agent: MOBaselinesAgent,  grounding_network: RewardVectorModule, alignment_layer_class=ConvexAlignmentLayer, alignment_layer_kwargs: Dict[str, Any] = {},
        ...
        self.value_system_per_cluster = [create_alignment_layer(
            None, alignment_layer_class, alignment_layer_kwargs) for _ in range(Lmax)]
        ...
        if isinstance(grounding_network, RewardVectorModule):
            ...
            assert isinstance(
                self.grounding_network, RewardVectorModule), "Grounding network must be an instance of RewardVectorModule"

            self.train_reward_net = EnsembleRewardVectorModule(
                [self.grounding_network.copy_new() for _ in range(n_rewards_for_ensemble)])
        else:
            assert isinstance(
                grounding_network, EnsembleRewardVectorModule), "Grounding network must be an instance of EnsembleRewardVectorModule"
            ...
            # Use the first reward vector as the grounding network referent
            self.grounding_network = self.train_reward_net.rewards[0]
        self.optim: VSLOptimizer = optim_class(**optim_kwargs, params_gr=set(self.train_reward_net.parameters()), params_vs=set(
            p for v in self.value_system_per_cluster for p in v.parameters()), n_values=self.grounding_network.num_outputs)
    ...
    def update_assignment_from_trained_networks(self, reference_assignment: ClusterAssignment, trained_reward_net, weights, only_grounding: bool = False):
        if not only_grounding or reference_assignment.Lmax != len(weights):
            for c in range(len(reference_assignment.weights_per_cluster)):
                reference_assignment.weights_per_cluster[c].load_state_dict(
                    deepcopy(reference_assignment.weights_per_cluster[c].state_dict()))
        else:
            reference_assignment.weights_per_cluster = weights
        if only_grounding:
            assert set(reference_assignment.value_systems) == set(self.get_value_systems(
            )), f"Static weights mismatch: {reference_assignment.weights_per_cluster} vs {self.value_system_per_cluster}"
        assert len(
            weights) == reference_assignment.Lmax, f"Value systems length mismatch: {len(weights)} vs {reference_assignment.Lmax}"
        ...
        if isinstance(reference_assignment.grounding, EnsembleRewardVectorModule):
            assert isinstance(reference_assignment.grounding,
                              EnsembleRewardVectorModule)
            assert len(trained_reward_net.rewards) == len(
                reference_assignment.grounding.rewards), f"Grounding network models length mismatch: {len(trained_reward_net.rewards)} vs {len(reference_assignment.grounding.rewards)}"

        reference_assignment.grounding.load_state_dict(
            deepcopy(trained_reward_net.state_dict()))
    ...
    def update_training_networks_from_assignment(self, value_system_per_cluster: List[LinearAlignmentLayer]=None, grounding: VectorModule=None, reference_assignment: ClusterAssignment=None, only_grounding: bool = False):
        ...
        with th.no_grad():
            if len(value_system_per_cluster) < reference_assignment.Lmax:
                value_system_per_cluster.extend([deepcopy(reference_assignment.weights_per_cluster[0]) for _ in range(
                    reference_assignment.Lmax - len(value_system_per_cluster))])
            ...
            if not only_grounding or reference_assignment.Lmax != len(self.value_system_per_cluster):
                for c in range(len(reference_assignment.weights_per_cluster)):
                    value_system_per_cluster[c].load_state_dict(
                        deepcopy(reference_assignment.weights_per_cluster[c].state_dict()))
            else:
                value_system_per_cluster = self.value_system_per_cluster

            assert len(
                value_system_per_cluster) == reference_assignment.Lmax, f"Value systems length mismatch: {len(value_system_per_cluster)} vs {reference_assignment.Lmax}"
            ...
            if isinstance(grounding, EnsembleRewardVectorModule):
                assert isinstance(reference_assignment.grounding,
                                  EnsembleRewardVectorModule)
                assert len(grounding.rewards) == len(
                    reference_assignment.grounding.rewards), f"Grounding network models length mismatch: {len(grounding.rewards)} vs {len(reference_assignment.grounding.rewards)}"
    ...
    def train_algo(self,
        ...
        if self.use_wandb:
            ...
            wandb.init(project="PVSL", dir=run_dir, name=experiment_name, tags=tags, config={
                       "algo": algo, "env_name": env_name, **kwargs, **self.init_config()}, reinit=True, )
        ...
        try:
            if algo == 'pc':
                ret = self.train_pc(experiment_name=experiment_name,
                                    dataset=dataset, resume_from=resume_from, **kwargs)
            elif algo == 'pbmorl':
                self.static_weights = True
                kwargs['static_weights'] = True
                ret = self.train_pbmorl(experiment_name=experiment_name, dataset=dataset,
                                        clustered_variant=False, resume_from=resume_from, **kwargs)
            elif algo == 'cpbmorl':
                ret = self.train_pbmorl(experiment_name=experiment_name, dataset=dataset,
                                        clustered_variant=True, resume_from=resume_from, **kwargs)
            
            elif algo == 'apbmorl':
                ret = self.train_apbmorl(experiment_name=experiment_name, dataset=dataset, resume_from=resume_from, **kwargs)
    ...
    def train_apbmorl(self, experiment_name: str, dataset: VSLPreferenceDataset, max_iter: int, K: int, H: int, 
                       Ns: int, Nw: int, initial_reward_learning_iterations: int,
                     max_reward_buffer_size: int, m_steps_per_cycle: int, policy_train_kwargs: Dict,
                     discount_factor_preferences, use_running_dataset_in_assignment: bool = False,
                     batch_size_per_agent: int = None, lexicographic_vs_first: bool = None,
                     resume_from=0):
        ...
        self.mobaselines_agent: MOBaselinesAgent = self.initialize_policy(
            self.train_env, eval_env=self.eval_env, algo='pbmorl')
        train_Agent: EnvelopePBMORL|EnvelopeClusteredPBMORL  = self.mobaselines_agent.agent_mobaselines
        ...
        for global_iter in range(resume_from, max_iter):
            ...
            if global_iter == 0:
                ...
                with th.no_grad():
                    ...
                    best_assignments_list = ClusterAssignmentMemory(
                        max_size=1, n_values=dataset.n_values)
                    self.repopulate_assignment_list(
                        dataset, best_assignments_list, evenly_spaced=True, one_agent_one_vs=True)
            ...
            assert self.Lmax == dataset.n_agents
            assert self.current_assignment.Lmax == dataset.n_agents
            assert self.current_assignment.L == self.Lmax
            ...
            train_Agent.train(Ns=Ns, Nw=Nw, K=K, eval_env=self.eval_env,
                              ...
                              reward_train_callback=partial(self._clustered_pbmorl_reward_train_callback,
                                                            global_iter=global_iter,
                                                            lexicographic_vs_first=lexicographic_vs_first,em_cycles_per_iteration=1,
                                                            batch_size_per_agent=batch_size_per_agent,
                                                            max_iter=max_iter,
                                                            initial_reward_learning_iterations=initial_reward_learning_iterations,
                                                            initial_exploration_rate=None,
                                                            mutation_prob=None,mutation_scale=None,
                                                            batch_size_in_running_dataset_for_cluster_assignment=None,
                                                            ...
                                                            allow_clusterize=False,
                                                            initial_m_steps_per_cycle=m_steps_per_cycle,
            ...
            self.wandb_log_cluster_metrics(
                    self.current_assignment, global_iter, 'train')
            self.mobaselines_agent.policy.set_agent_baselines(train_Agent)
            assert self.current_assignment.grounding == self.train_reward_net
            self.mobaselines_agent.set_reward_vector_function(
                self.train_reward_net)
            ...
            assert best == self.current_assignment, f"Best assignment mismatch {best} vs \n {self.current_assignment}"
            assert best.Lmax == self.Lmax == len(
                self.value_system_per_cluster) == self.current_assignment.Lmax == dataset.n_agents
        ...
        _, historic, _, _, _ = PVSL.load_state(
        ...
        for global_iter in range(resume_from, max_iter):
            if global_iter == 0:
                ...
                with th.no_grad():
                    ...
                    self.repopulate_assignment_list(
                        dataset, best_assignments_list, evenly_spaced=True)
    ...
    def train_pc(self, experiment_name: str, dataset: VSLPreferenceDataset, max_iter: int,
        ...
        self.policy_train_kwargs = policy_train_kwargs
        policy_iterations = self.policy_train_kwargs[
            'total_timesteps'] // max_iter if online_policy_update else self.policy_train_kwargs['total_timesteps']
        self.policy_train_kwargs['total_timesteps'] = policy_iterations
        self.mobaselines_agent: MOBaselinesAgent = self.initialize_policy(
            self.train_env, eval_env=self.eval_env, algo='pc')
        ...
        try:
            while global_iter < max_iter:
                ...
                with th.no_grad():
                    if global_iter == 0:
                        ...
                        self.repopulate_assignment_list(
                            dataset, best_assignments_list)
                    # print(len(best_assignments_list), best_assignments_list.max_size)
                    elif resume_from == global_iter:
                        best_assignments_list, historic, self.mobaselines_agent, final_global_step, config = PVSL.load_state(
                            ename=experiment_name, agent_name=self.mobaselines_agent.name, ref_env=self.train_env, ref_eval_env=self.eval_env)
                        assert global_iter < len(
                            historic), f"Historic does not contain the supplied resume_from ({resume_from}), maximum is {len(historic)}"
                ...
                PVSL.save_state(ename=experiment_name, best_assignments_list=None, historic_dict={
                                global_iter: best}, policy_per_cluster=None, global_step=self.global_step, save_config=self.init_config())
            ...
            PVSL.save_state(ename=experiment_name, best_assignments_list=best_assignments_list, historic_dict=None,
                            policy_per_cluster=self.mobaselines_agent, global_step=self.global_step, save_config=self.init_config())
            if not online_policy_update:
                ...
                self.update_training_networks_from_assignment(
                    self.value_system_per_cluster, self.train_reward_net, self.current_assignment, only_grounding=self.static_weights)
                self.mobaselines_agent = self.update_policy(
                    dataset, iterations=policy_iterations)
        except Exception as e:
            wandb.finish(exit_code=e.__cause__, quiet=False)
            print(
                f"Exception during training: {e}. Saving state before exiting.")
            raise e
        finally:
            PVSL.save_state(ename=experiment_name, best_assignments_list=best_assignments_list, historic_dict=None,
                            policy_per_cluster=self.mobaselines_agent, global_step=self.global_step, save_config=self.init_config())
    ...
    def repopulate_assignment_list(self, dataset, best_assignments_list, evenly_spaced=False, one_agent_one_vs=False):
        best_assignments_list.initializing = True
        while len(best_assignments_list) < best_assignments_list.max_size:
            ca = generate_random_assignment(dataset, l_max=self.Lmax, alignment_layer_class=self.alignment_layer_class, alignment_layer_kwargs=self.alignment_layer_kwargs,
                                            ref_grounding=self.train_reward_net, seed=np.random.randint(0, 10000), evenly_spaced=evenly_spaced)
            
            if one_agent_one_vs:
                assert self.Lmax == dataset.n_agents
                assert ca.Lmax == dataset.n_agents
                assert ca.L == self.Lmax
                
            ca.recalculate_discordances(
                dataset, indifference_tolerance=self.loss.model_indifference_tolerance)
            ca.optimizer_state = self.optim.get_state(copy=True)
            self.update_training_networks_from_assignment(
                self.value_system_per_cluster, self.train_reward_net, ca, only_grounding=self.static_weights)
    ...
    def wandb_log_cluster_metrics(self, assignment, global_iter, mode='train'):
        if self.use_wandb:
            wandb.log({f"{mode}/conciseness": assignment.conciseness_vs(), f"{mode}/representativeness": assignment.representativity_vs(),
                      f"{mode}/coherence": float(np.mean(assignment.coherence()))}, step=global_iter)

    def wandb_log_losses(self, global_iter, train_losses, mode='train'):
        if self.use_wandb:
            wandb.log({f"{mode}/loss_vs": train_losses[0], f"{mode}/loss_gr": train_losses[1],
                      f"{mode}/vs_norm": train_losses[2], f"{mode}/gr_norm": train_losses[3]}, step=global_iter)
        ...
        with th.no_grad():
            ...
            if historic_dict is not None:
                for global_iter, assignment in historic_dict.items():
                    ...
                    assignment.save(os.path.join(
                        folder, 'historic'), f"assignment_{global_iter}")
            # Save policy
            if policy_per_cluster is not None:
                policy_per_cluster.save(path=os.path.join(
                    folder, "policy"), full_save=True)
            # Save global step
    ...
    def load_state(ename, agent_name, ref_env, ref_eval_env):
        ...
        try:
            ...
            print(f"Error loading best_assignments_list: {e}")
            best_assignments_list = None
        historic = {}
        ...
        historic_list = [v for k, v in sorted(
            historic.items(), key=lambda item: item[0], reverse=False)]
        # assert historic_list[0] == historic[0]
        policy = MOBaselinesAgent.load(env=ref_env, eval_env=ref_eval_env, path=os.path.join(
            folder, "policy"), name=agent_name)
        # load global step
    ...
    def initialize_policy(self, env, eval_env, algo='pc', **kwargs):
        """
        Initialize a policy to be learned via a PbMORL/Preference Comparisons approach.
        """
    ...
    def selection(self, best_assignments_list: ClusterAssignmentMemory, lexicographic_vs_first, iter_now, num_iterations, initial_exploration_rate, mutation_scale, mutation_prob, dataset) -> ClusterAssignment:
        if len(best_assignments_list) < best_assignments_list.max_size:
            best_assignments_list.initializing = True
            while len(best_assignments_list) < best_assignments_list.max_size:
                original, is_protected = best_assignments_list.get_random_weighted_assignment(
                    consider_only_unexplored=False, lexicographic_vs_first=lexicographic_vs_first)
                mut = generate_mutated_assignment(
                    original.copy(), mutation_scale, mutation_prob)

                mut.recalculate_discordances(
                    dataset, indifference_tolerance=self.loss.model_indifference_tolerance)
        ...
        with th.no_grad():
            ...
            if random.random() > self.exploration_rate(initial_exploration_rate=initial_exploration_rate, iter_now=iter_now, num_iterations=num_iterations):  # Example: 1000 is the total number of iterations
                ...
                mutation = generate_mutated_assignment(
                    original.copy(), mutation_scale, mutation_prob)
                mutation.explored = False
                if initial_exploration_rate == 0.0:
                    raise ValueError(
                        "Initial exploration rate is 0.0, cannot mutate.")
                return mutation, original, True, False

    def get_cluster_colors(self, mode='colorama', **kwargs):
        self.cluster_colors_vs[mode] = COLOR_PALETTE.get(
            mode)(len(self.value_system_per_cluster), **kwargs)
        assert len(
            self.cluster_colors_vs[mode]) == self.Lmax, f"Cluster colors length mismatch: {len(self.cluster_colors_vs[mode])} vs {self.Lmax}"
    ...
    def cluster_assignment(self, dataset: VSLPreferenceDataset, running_dataset: FixedLengthVSLPreferenceDataset = None, batch_size_in_running_dataset_for_cluster_assignment=0, qualitative_cluster_assignment=False):
        ...
        if self.debug_mode:
            print(
                "Clustering assignments based on current value systems and grounding network...")
                ...
                for aid in dataset.fidxs_per_agent.keys():
                    if aid in running_dataset.fidxs_per_agent:
                        ...
                        if batch_size_in_running_dataset_for_cluster_assignment > size_data:
                            ...
                            frags_per_aid_running[aid] = np.random.choice(
                                running_dataset.fidxs_per_agent[aid], size=batch_size_in_running_dataset_for_cluster_assignment, replace=False)

            grounding1 = self.train_reward_net.forward(fragments1)
            grounding2 = self.train_reward_net.forward(fragments2)
            fkeys = list(frags_per_aid_running.keys())
            for aid in dataset.agent_data.keys():

                fidxs = dataset.fidxs_per_agent[aid]
                if aid in frags_per_aid_running.keys():
                    fidxs_running = frags_per_aid_running[aid]
                    grounding1_running = self.train_reward_net.forward(
                        running_dataset.fragments1[fidxs_running])
                    grounding2_running = self.train_reward_net.forward(
                        running_dataset.fragments2[fidxs_running])
                # print("AGENT IDS", aid, fidxs, grounding1[fidxs][0:10])
                agent_preferences = dataset.preferences[fidxs]
                if aid in frags_per_aid_running.keys():
                    agent_preferences = np.concatenate(
                        (agent_preferences, running_dataset.preferences[fidxs_running]))
                    assert agent_preferences.shape == (len(
                        fidxs) + len(fidxs_running),), f"Preference shape mismatch: {agent_preferences.shape} vs {len(fidxs)} + {len(fidxs_running)}"
                aid_likelihood_per_vs_cluster = [1.0]*self.Lmax
                for c in range(self.Lmax):
                    vs = self.value_system_per_cluster[c]
                    # TODO this can be more efficient...

                    probs = probability_BT(vs.forward(grounding1[fidxs]), vs.forward(
                        # shape: (n_fragments, n_values)
                        grounding2[fidxs])).squeeze(-1)
                    if aid in frags_per_aid_running.keys():
                        probs_running = probability_BT(vs.forward(
                            grounding1_running), vs.forward(grounding2_running)).squeeze(-1)
                    ...
                    aid_likelihood_per_vs_cluster[c] = likelihood_x_is_target(probs.detach(), util.safe_to_tensor(gt_probs).detach(
                    ), mode='th', slope=0.1, adaptive_slope=False, qualitative_mode=qualitative_mode, indifference_tolerance=self.loss.model_indifference_tolerance)
                ...
                if qualitative_mode:
                    ...
                    best_clusters = [i for i, v in enumerate(
                        aid_likelihood_per_vs_cluster) if v == max_likelihood]
                    ...
                    candidate_clusters = [c for c, cnt in zip(
                        best_clusters, counts) if cnt == max_count]
                ...
                if len(self.value_system_per_cluster) > 1 and self.debug_mode:
                    ...
                    print(
                        f"), {cluster_colors_vs[best_cluster]} {self.value_system_per_cluster[best_cluster].get_alignment_layer()[0]} - VS Cluster {best_cluster}: {aid_likelihood_per_vs_cluster[best_cluster]}, {Style.RESET_ALL}")

            changed = True
            
            while changed:
                changed = False
                for cl, ags in enumerate(assignment_vs):
                    if len(ags) == 0:
                        continue
                    vsc = self.value_system_per_cluster[cl].get_alignment_layer()[
                        0].detach().numpy()
                    for cl2, ags2 in enumerate(assignment_vs):
                        if len(ags2) == 0 or cl2 == cl:
                            continue
                        vsc2 = self.value_system_per_cluster[cl2].get_alignment_layer()[
                            0].detach().numpy()
                        if np.max(abs(vsc - vsc2)) <= 0.05:
                            changed = True
                            ags = assignment_vs[cl]
                            ags2 = assignment_vs[cl2]
                            print("Doing it now", vsc, vsc2)
                            print("Merging clusters", cl, "and", cl2, "with agents:")
                            print(cl, ags, cl2, ags2)
                            print("??? should be equal", assignment_vs[cl2], ags2)
                            if len(ags) >= len(ags2):  # we choose the most popular weights
                                print("Doing it now 1")
                                ...
                                for ag in ags2:
                                    agent_to_vs_cluster_assignments[ag] = cl
                                # print(agent_to_vs_cluster_assignments)
                                # print(assignment_vs)
                                print("BEFORE1", self.value_system_per_cluster[cl2])
                                self.value_system_per_cluster[cl2].set_weights(random_weights(
                                    dataset.n_values, 1, dist='gaussian', seed=np.random.randint(0, 1000)))
                                print("AFTER1", self.value_system_per_cluster[cl2])
                            else:
                                print("Doing it now 2")
                                ...
                                # print(agent_to_vs_cluster_assignments)
                                # print(assignment_vs)
                                # print("BEFORE", self.value_system_per_cluster[cl])
                                self.value_system_per_cluster[cl].set_weights(random_weights(
                                    dataset.n_values, 1, dist='gaussian', seed=np.random.randint(0, 1000)))
                                # print("AFETR", self.value_system_per_cluster[cl])
                            print("Result:")
                            print("CL", cl, ":\n", assignment_vs[cl], "\n", cl2, ":\n", assignment_vs[cl2])
                            for ag in agent_to_vs_cluster_assignments.keys():
                                print("AG", ag, "->", agent_to_vs_cluster_assignments[ag])
            check_grounding_value_system_networks_consistency_with_optim(
    ...
    def train_reward_models(self, dataset, global_step, n_optim_steps, batch_size_reward_buffer=32, clusterizing=False):
        ...
        self.set_mode('train')
        vs_prev = deepcopy(self.get_value_systems())
        vs_prev2 = self.current_assignment.value_systems

        assert all([tuple(v) == tuple(v2) for v, v2 in zip(vs_prev, vs_prev2)]
                   ), f"Value systems in current assignment and trainer do not match.{vs_prev} vs {vs_prev2}"
    ...
    def train_reward_models_old(self, dataset: VSLPreferenceDataset, global_step, n_optim_steps, batch_size_per_agent='full', transitions=True, alternative_dataset=None, percentage_from_alternative_dataset=0.2):
        ...
        # TODO: alternative dataset. train with base dataset as alternative dataset a percentage of times.
        self.set_mode('train')
        train_losses = []
        print("SIZE DATA", len(dataset), "NAGENTS",
        ...
        for sum_ensemble in range(self.train_reward_net.n_models):
            ...
            for i in range(n_optim_steps):
                ...
                with th.no_grad():
                    dataset_b, fids_per_agent = dataset.select_batch(
                        batch_size_per_agent, transitions=transitions, device=self.train_reward_net.device)

                    fragments1, fragments2, preferences, preferences_with_grounding, agent_ids = dataset_b
                    print("Sampled", len(fragments1),
                          "fragments, agents:", len(OrderedSet(agent_ids)))
...
"""
                self.loss.gradients(scalar_loss=loss_vs,
                ...
                train_losses.append((loss_vs.detach().item(), loss_gr.detach(
                ).item(), vs_norm.detach(), gr_norm.detach()))
    ...
    def update_policy(self, dataset: VSLPreferenceDataset, iterations=10):
        ...
        if self.mobaselines_agent.is_single_objective:
            for c, agents in self.current_assignment.active_vs_clusters():
                ...
                self.mobaselines_agent.set_weights(
                    self.value_system_per_cluster[c].get_alignment_layer()[0][0], override_existent=c)
                self.mobaselines_agent.fit(
                    eval_env=self.eval_env, weight=self.value_system_per_cluster[c])
...
class EnvelopeClusteredPBMORL(EnvelopePBMORL):
    def __init__(self, env, buffer_with_weights, cluster_weights_in_envelope_prob, cluster_assignment=None, learning_rate=0.0003, estimated_horizon=10, initial_epsilon=0.01, final_epsilon=0.01, epsilon_decay_steps=None, tau=1, target_net_update_freq=200, buffer_size=..., net_arch=..., batch_size=256, learning_starts=100, gradient_updates=1, gamma=0.99, max_grad_norm=1, envelope=True, num_sample_w=4, per=False, per_alpha=0.6, initial_homotopy_lambda=0, final_homotopy_lambda=1, homotopy_decay_steps=None, project_name="MORL-Baselines", experiment_name="Envelope", wandb_entity=None, log=True, seed=None, device="auto", activation=None, group=None, masked=False, relabel_buffer=True, reward_vector_function=None):
        super().__init__(env, buffer_with_weights, True, learning_rate, estimated_horizon, initial_epsilon, final_epsilon, epsilon_decay_steps, tau, target_net_update_freq, buffer_size, net_arch, batch_size, learning_starts, gradient_updates, gamma, max_grad_norm,
                         envelope, num_sample_w, per, per_alpha, initial_homotopy_lambda, final_homotopy_lambda, homotopy_decay_steps, project_name, experiment_name, wandb_entity, log, seed, device, activation, group, masked, relabel_buffer, reward_vector_function)
    ...
    def sample_eval_weights(self, n):
        if self.running_assignment is None:
            ...
            print(n, len(self.running_assignment.value_systems))
            if n < len(self.running_assignment.value_systems):
                ...
                weights_base = set(
                    deepcopy(self.running_assignment.value_systems))
                eqweights = set([tuple(w)
                                for w in super().sample_eval_weights(n=n)])
    ...
    def reward_training(self, Ns, Nw, H, gamma_preferences, reward_train_callback, max_reward_buffer_size, qualitative_preferences=True):
        return_dict = super().reward_training(Ns, Nw, H, gamma_preferences, reward_train_callback,
                                              max_reward_buffer_size, qualitative_preferences=qualitative_preferences)
    ...
    def sample_traj_from_dataset(self, dataset: VSLPreferenceDataset, Ns, gamma):
        ...
        idx = np.random.choice(np.arange(
            len(fragments)), size=Ns, replace=True if Ns > len(fragments) else False)
        ...
        for f in frag:
            ...
            for vi in range(dataset.n_values):
                drews[-1][vi] = rollout.discounted_sum(
                    f.v_rews[vi],  gamma=gamma)
                drews_real[-1][vi] = rollout.discounted_sum(
                    rews_real[vi],  gamma=gamma)
        ...
        assert v_rews.shape[0:1] == (
            Ns, ), f"Rewards shape mismatch: {v_rews.shape} vs {(dataset.n_values, Ns)}"
        assert v_rews_real.shape[0:1] == (
            Ns, ), f"Rewards shape mismatch: {v_rews_real.shape} vs {(dataset.n_values, Ns)}"
        assert drews.shape[0:2] == (
            Ns, dataset.n_values), f"Rewards shape mismatch: {drews.shape} vs {(dataset.n_values, Ns)}"
    ...
    def train(self, total_timesteps, Ns, Nw, H, K, gamma_preferences, reward_train_callback, dataset=None, eval_env=None, ref_point=None, known_pareto_front=None, weight=None, total_episodes=None, reset_num_timesteps=True, eval_freq=10000, num_eval_weights_for_front=100, num_eval_episodes_for_front=5, num_eval_weights_for_eval=50, qualitative_preferences=True, reset_learning_starts=False, verbose=False, max_reward_buffer_size=None, reference_assignment: ClusterAssignment = None):
        ...
        for i, w in enumerate(reference_assignment.weights_per_cluster):
            ...
            assert not any([p.requires_grad for p in w.parameters(
            )]), "Weights in reference assignment must be frozen (requires_grad=False)."
    ...
    def overseer_new_preferences(self, Ns, Nw, H, gamma_preferences, max_reward_buffer_size, discordance_based_agent_selection=False, qualitative_preferences=True):
        ...
        with th.no_grad():
            ...
            fragments1, fragments2, real_v_rew1, real_v_rew2, lear_v_rew1, lear_v_rew2, dsreal_v_rew1, dsreal_v_rew2, dslear_v_rew1, dslear_v_rew2 = self.replay_buffer.sample_trajs(
                ns=Ns, gamma=gamma_preferences, get_rewards_orig=True, device=self.device, to_tensor=True, H=H)
            """
            from_dataset_2 = False
            if from_dataset_2:
                random_positions = np.random.choice(
                    len(fragments1[0]), size=int(0.5*len(fragments1[0])), replace=False)
                fragments2_, real_v_rew2_, lear_v_rew2_, dsreal_v_rew2_, dslear_v_rew2_ = self.sample_traj_from_dataset(
                    self.base_dataset, Ns, gamma=gamma_preferences)
                for i in range(len(fragments2)):
                    fragments2[i][random_positions] = fragments2_[
                        i][random_positions]
                real_v_rew2[random_positions] = real_v_rew2_[random_positions]
                lear_v_rew2[random_positions] = lear_v_rew2_[random_positions]
                dsreal_v_rew2[random_positions] = dsreal_v_rew2_[
                    random_positions]
                dslear_v_rew2[random_positions] = dslear_v_rew2_[
                    random_positions]"""
            ...
            assert len(fragments1[0]) == len(
                fragments2[0]), f"Fragments shape mismatch: {len(fragments1)} vs {len(fragments2[0])}"
            assert len(real_v_rew1) == len(
                fragments1[0]), f"Original rewards and rewards length mismatch: {len(real_v_rew1)} vs {len(fragments1[0])}"
            assert len(real_v_rew2) == len(
                fragments2[0]), f"Original rewards and rewards length mismatch: {len(real_v_rew2)} vs {len(fragments2[0])}"
            assert dsreal_v_rew1.shape == (
                Ns, self.reward_dim), f"Rewards shape mismatch: {dsreal_v_rew1.shape} vs {(Ns, self.reward_dim)}"
            assert dsreal_v_rew2.shape == (
                Ns, self.reward_dim), f"Rewards shape mismatch: {dsreal_v_rew2.shape} vs {(Ns, self.reward_dim)}"
            assert dslear_v_rew1.shape == (
                Ns, self.reward_dim), f"Rewards shape mismatch: {dslear_v_rew1.shape} vs {(Ns, self.reward_dim)}"
            assert dslear_v_rew2.shape == (
                Ns, self.reward_dim), f"Rewards shape mismatch: {dslear_v_rew2.shape} vs {(Ns, self.reward_dim)}"
            ...
            """if max_reward_buffer_size < len(self.running_dataset) + Ns * Nw:
                print("\033[95mPOPPPED",  len(self.running_dataset) - max_reward_buffer_size + Ns *
                      Nw, "out of", len(self.running_dataset), "max", max_reward_buffer_size, "\033[0m")
                self.running_dataset.pop(
                    len(self.running_dataset) - max_reward_buffer_size + Ns * Nw)
                exit(0)"""
            # Gets a new agent for each cluster.
            shuffled_assignments = np.random.permutation(np.asarray(
                self.running_assignment.assignments_vs, dtype=list))
            ...
            while counter < Nw:
                rep += 1
                is_incomplete_assignment = self.running_dataset.n_agents < self.running_assignment.n_agents
                agents = self.running_dataset.fidxs_per_agent.keys()
                assert len(set(self.running_dataset.agent_ids)) ==  self.running_dataset.n_agents 
                assert set(agents).issubset(set(self.running_assignment.agent_to_vs_cluster_assignments.keys())), "Some agents in the dataset are not in the running assignment."
                assert set(agents) == set(self.running_dataset.agent_ids).difference(set([0])), f"Mismatch between agents in dataset and fidxs_per_agent. fidxs_per_agent: {agents}\n dataset agents: {set(self.running_dataset.agent_ids)}"
                
                
                candidates = shuffled_assignments[cluster_idx_pointer %
                                                  self.running_assignment.Lmax]
                candidates = [c for c in candidates if c not in already_picked]
                cluster_idx_pointer += 1
                
                print(counter, Nw, "Picked candidates from cluster",)
                if is_incomplete_assignment:
                    print("C with agents??", candidates)
                    candidates = [c for c in candidates if c not in agents]
                    print("C without agents ??", candidates)
                    print("A??", agents, " n agents in dataset", self.running_dataset.n_agents, len(agents))
                    if len(candidates) == 0 and rep > Nw*self.running_assignment.Lmax:
                        input("No more candidates available in incomplete assignment.")
                ...
                if not discordance_based_agent_selection:
                    ...
                    for an in candidates:
                        if self.running_assignment.intra_discordances_vs_per_agent[an] > max_discordance:
                            aname = an
                            max_discordance = self.running_assignment.intra_discordances_vs_per_agent[
                                an]
                already_picked.add(aname)
                ...
                assert dsrw1.shape == dsrw2.shape == (
                    Ns,), f"Rewards shape mismatch: {dsrw1.shape} vs {dsrw2.shape} vs {(Ns,)}"
                preferences_w = probs_to_label(probability_BT(dsrw1, dsrw2))

                preferences_with_grounding = th.zeros(
                    (len(preferences_w), self.reward_dim)).to(self.device)
                ...
                assert th.all((dsrw1 >= dsrw2) == (preferences_w >= 0.5)), "preferences_w should be 1 when dsrw1 > dsrw2"
                    """
                assert preferences_with_grounding.shape == (len(preferences_w), self.reward_dim), f"Preferences with grounding shape mismatch: {preferences_with_grounding.shape} vs {(len(preferences_w), self.reward_dim)}"
                for vi in range(self.reward_dim):
                    preferences_with_grounding[:, vi] = probs_to_label(
                        probability_BT(dsreal_v_rew1[:, vi], dsreal_v_rew2[:, vi]))
                ...
                for i in range(len(fragments1[0])):
                    s1, a1, n1, d1 = (fragments1[stand][i]
                                      for stand in range(len(fragments1)))
                    s2, a2, n2, d2 = (fragments2[stand][i]
                                      for stand in range(len(fragments2)))
                    ...
                    assert fobs1.shape == (
                        s1.shape[0] + 1, *s1.shape[1:]), f"Fragment obs shape mismatch: {fobs1.shape} vs {s1.shape[0] + 1}"
                ...
                self.running_dataset.push(np.asarray(fpairs_w, dtype=tuple), preferences=preferences_w,
                                          ...
                                          agent_data={
                                              aname: self.base_dataset.agent_data[aname]},
                                          agent_ids=[aname for _ in range(len(fpairs_w))])
                rt = time.time()
                print(
                    f"\033[35mPush to running dataset took {rt-t:.4f} seconds\033[0m")
    ...
    def sample_new_weights(self, n, random=False):
        if self.running_assignment is None:
            weights = random_weights(
                dim=self.reward_dim, n=n, dist="gaussian", rng=self.np_random)
        else:
            weights = []  
            # [self.cluster_weights_in_envelope_prob]% of times sample from running_assignment
            # Unless # [self.cluster_weights_in_envelope_prob] = "random"
            for _ in range(n):
                if self.cluster_weights_in_envelope_prob == 1.0:
                    idx = np.random.choice(
                        len(self.running_assignment.value_systems_active))
                    w = self.running_assignment.value_systems_active[idx]
                elif self.cluster_weights_in_envelope_prob == "all":
                    idx = np.random.choice(
                        len(self.running_assignment.value_systems))
                    w = self.running_assignment.value_systems[idx]
                elif np.random.rand() < self.cluster_weights_in_envelope_prob:
                    idx = np.random.randint(
                        0, len(self.running_assignment.value_systems_active))
                    assert len(self.running_assignment.value_systems_active) == len(
                        [_ for asg in self.running_assignment.assignments_vs if len(asg) > 0])

                    w = self.running_assignment.value_systems_active[idx]
                elif self.cluster_weights_in_envelope_prob == "random":
                    w = random_weights(dim=self.reward_dim,
                                       n=1, dist="gaussian", rng=self.np_random)
                else:
                    raise ValueError(f"Invalid value for parameter \'cluster_weights_in_envelope_prob\': {self.cluster_weights_in_envelope_prob}. Should be float in [0,1] or \'all\' or \'random\'.")
                weights.append(w)
            assert len(
                weights) == n, f"Weights length mismatch: {len(weights)} vs {n}"
            if n == 1:
                weights = weights[0]
        return weights

class EnvelopeAgentPBMORL(EnvelopeClusteredPBMORL):
    ...
    def overseer_new_preferences(self, Ns, Nw, H, gamma_preferences, max_reward_buffer_size, discordance_based_agent_selection=False, qualitative_preferences=True):
        ...
        with th.no_grad():
            ...
            """if max_reward_buffer_size < len(self.running_dataset) + Ns * Nw:
                ...
                exit(0)"""
            # Gets a new agent for each cluster.
            candidates = list(self.running_assignment.agent_to_vs_cluster_assignments.keys())
            if not discordance_based_agent_selection:
                    # Option 1: ask 6 (Nw) agents randomly from different clusters. Add that to the reward training dataset.
                    shuffled_agents = np.random.permutation(
                    candidates)
            else:
                # Order candidates by decreasing intra_discordances_vs_per_agent (highest first)
                scores = [(an, self.running_assignment.intra_discordances_vs_per_agent[an]) for an in candidates]
                # sort by score descending (highest discordance first)
                scores.sort(key=lambda x: x[1], reverse=True)
                shuffled_agents = [an for an, _ in scores]
            assert len(shuffled_agents)  == self.running_assignment.n_agents, f"Shuffled agents length mismatch: {len(shuffled_agents)} vs {self.running_assignment.n_agents}"   
    
            agent_idx_pointer = 0
            already_picked = set()
            while agent_idx_pointer < Nw:
                aname = shuffled_agents[agent_idx_pointer]
                assert aname not in already_picked
                already_picked.add(aname)
                agent_idx_pointer += 1
