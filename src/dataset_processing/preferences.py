    ...
    with open(csv_path, 'r') as csvfile:
        ...
        for ir, row in enumerate(reader):
            ...
            if discounted_sums[traj1] != 0.0:
                #print(cr1, discounted_sums[traj1])
                assert np.allclose(discounted_sums[traj1], cr1)
            discounted_sums[traj1] = cr1

            if discounted_sums[traj2] != 0.0:
                assert np.allclose(discounted_sums[traj2], cr2)
        ...
        with open(csv_path, 'r') as csvfile:
            ...
            for ir, row in enumerate(reader):
                ...
                if discounted_sums_per_grounding[vi, traj1] != 0.0:
                    assert np.allclose(discounted_sums_per_grounding[vi, traj1], cr1)
                discounted_sums_per_grounding[vi, traj1] = cr1

                if discounted_sums_per_grounding[vi, traj2] != 0.0:
                    assert np.allclose(discounted_sums_per_grounding[vi, traj2], cr2)
