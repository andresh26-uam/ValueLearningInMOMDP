...
class FireFightersEnvMO(gym.Env):
    ...
    def __init__(self, feature_selection=FeatureSelectionFFEnv.ONE_HOT_FEATURES, horizon=50, initial_state_distribution='random', **kwargs):
        self.horizon = horizon
        self.real_env = HighRiseFireEnv()
        self.n_values = 2  # Two objectives: Professionalism and Proximity

        self.reward_space = spaces.Box(low=-50.0, high=50.0, shape=(self.n_values,), dtype=np.float32)
...
, (self.state_dim*self.action_dim,))
    ...
    def terminal(self, state: DiscreteSpaceInt, n_actions_taken: int) -> bool:
        ...
        assert self.horizon is not None, "Horizon must be set to check for terminal state."
        return n_actions_taken >= self.horizon or self.real_env.is_done(self.real_env.translate(state))
